...
| Water quality samples | Water Quality Portal (WQP) | Good | Implemented with `data/Station/search` and `data/Result/search`, using direct browser fetches for nearby monitoring stations and recent discrete sample results. The app labels WQP separately as discrete lab/sample data and uses it for recent bacteria and chemistry context, not live advisory status. |
| Official beach advisories | San Diego County Beach and Bay Water Quality Program (`sdbeachinfo`) | Mixed | Ocean Planner now reserves this as the authoritative county advisory source and scaffolds normalized advisory/event types plus curated location mapping hints. The public site is an OutSystems app at `https://cosdapps.sandiegocounty.gov/sdbeachinfo/`. Initial endpoint probing found public `screenservices` routes and county site/event models, but naive POSTs to those routes currently return `Invalid Login` unless the full app request/session shape is reproduced. Keep the static client on official-page fallback until a stable browser-callable request is validated. |
| Near-real-time water conditions | SCCOOS / ERDDAP Scripps Pier | Good | Implemented with the public SCCOOS ERDDAP `tabledap/scripps-pier-automated-shore-sta-1.json` endpoint for recent sensor observations including water temperature, salinity, chlorophyll, turbidity, pH, and dissolved oxygen. The app labels SCCOOS separately as near-real-time sensor context from Scripps Pier. |
