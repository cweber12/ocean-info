...
| Weather fallback | OpenWeather One Call 3.0 via Cloudflare Worker | Backend-only edge proxy | Implemented as selective fallback for hourly weather and wind when NOAA/NWS data is unavailable or incomplete. OpenWeather key is stored in Worker secrets, never in browser-visible `VITE_*` variables. |
